...
ui <- bslib::page_fluid(
  ...
  titlePanel("Welsh 2002 vs Welsh 2026 vs English 2016 Pay comparator v1.1.5"),
  sidebarLayout(
    sidebarPanel(
      width = 4,
      ...
      tagList(
        ...
        numericInput("enhanced_hours_eng", 
                     label = HTML("<strong>(England 2016)</strong> How many of those hours are enhanced/OOH? - plain time is 7am-9pm in England (due to the larger plain time range, this number should never be bigger for England)"), 
                     value = 8, min = 0, max = 56, step = 0.25),
...
server <- function(input, output, session) {
  ...
  output$premia_selector <- renderUI({
    ...
    # Set popover content based on grade
    popover_content <- if (grade %in% c("FY1", "FY2")) {
      "For Foundation Years, only the Academia premia is available. Please notify me if this is different for the welsh 2026 contract."
    } else {
      "For the Welsh 2026 OMFS premia, select any OMFS option. Generally you should only select up to two options. You can select more, but there don't seem to be any realistic combinations other than Academia + one other."
    }
