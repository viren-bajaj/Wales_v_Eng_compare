...
Banding <- data.frame(
	pay_band_mult_name = c("no band","1C","1B","1A","2B","2A","3","GP"),
...
pay_premia <- data.frame(
  ...
  f_premia = c(5216, 10690, 4347, 4347, 3260, 5216, 8693, 6520, 5216, 4347, 3726, 3260, 8693, 6520, 5216, 4347, 3726, 3260)
)
...
ui <- bslib::page_fluid(
  ...
  titlePanel("Welsh 2002 vs Welsh 2026 vs English 2016 Pay comparator"),
  sidebarLayout(
    sidebarPanel(
      width = 4,
      
      # England Grade Select
      ...
      h6(strong("Banding Options (Wales 2002 only)")),
      ...
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          radioButtons("pay_band", "What's your band?",
            choices = c("no band", "1C", "1B", "1A", "2B", "2A", "3", "GP"),
            selected = "no band",
            inline = TRUE
          )				
        )
...
# hours	  
      ...
      tagList(
        ...
        numericInput("enhanced_hours", label = "How many of those hours are enhanced/OOH?", 
                    value = 8, min = 0, max = 56, step = 0.25),
        checkboxInput("weekend_work", "Do you work weekends (english contract)?", value = TRUE),
        conditionalPanel(
      ...
      h6(strong("London Weighting (english contract)")),
...
server <- function(input, output, session) {
  ...
  vals <- reactiveValues(salary_html = NULL)
	...
	observe({
		...
		if (!is.null(total_hours) && !is.na(total_hours) && total_hours >= 0) {
			if (!is.null(current_enhanced) && !is.na(current_enhanced)) {
	...
	observeEvent(input$calculate, {   
		...
		if (is.null(input$enhanced_hours) || is.na(input$enhanced_hours)) {
			showNotification("Please enter a valid number for enhanced/OOH hours.", type = "error")
			return()
		}
		...
		selected_band <- if (!is.null(input$pay_band)) input$pay_band else "no band"
		...
		welsh_enhanced <- 1.5
		w_enhanced_pa <- input$enhanced_hours * w_base_hourly * welsh_enhanced * 52
		w_non_enhanced_pa <- basic_hours * w_base_hourly * non_enhanced_multiplier * 52
		...
		out <- c(out, sprintf("<li>Basic Pay (%s hrs): %s</li>", total_time, fmt(non_enhanced_pay)))
		...
		out <- c(out, "<p><strong>Breakdown:</strong></p><ul>")
		out <- c(out, sprintf("<li>Basic Pay: %s</li>", fmt(w_non_enhanced_pay)))
		if (w_enhanced_pay > 0) out <- c(out, sprintf("<li>Enhanced Pay (%s hrs @ 50%%): %s</li>", enhanced_time, fmt(w_enhanced_pay)))
		...
		out <- c(out, "<th style='text-align:right; padding:8px;'>Net Pay</th>")
		out <- c(out, "<th style='text-align:right; padding:8px;'>Difference vs England</th>")
		out <- c(out, "</tr>")
		
		out <- c(out, "<tr style='background-color:#f9f9f9;'>")
		out <- c(out, sprintf("<td style='padding:8px;'>England 2016</td>"))
		out <- c(out, sprintf("<td style='text-align:right; padding:8px;'>%s</td>", fmt(net_eng)))
		out <- c(out, "<td style='text-align:right; padding:8px;'>—</td>")
		out <- c(out, "</tr>")
		
		diff_w02 <- net_w_02 - net_eng
		pct_w02 <- (diff_w02 / net_eng) * 100
		color_w02 <- if(diff_w02 >= 0) "green" else "red"
		out <- c(out, "<tr>")
		out <- c(out, sprintf("<td style='padding:8px;'>Wales 2002</td>"))
		out <- c(out, sprintf("<td style='text-align:right; padding:8px;'>%s</td>", fmt(net_w_02)))
		out <- c(out, sprintf("<td style='text-align:right; padding:8px; color:%s;'>%s (%s%.2f%%)</td>", 
			color_w02, fmt(diff_w02), if(diff_w02 >= 0) "+" else "", pct_w02))
		out <- c(out, "</tr>")
		
		diff_w26 <- net_w_26 - net_eng
		pct_w26 <- (diff_w26 / net_eng) * 100
		color_w26 <- if(diff_w26 >= 0) "green" else "red"
		out <- c(out, "<tr style='background-color:#f9f9f9;'>")
		out <- c(out, sprintf("<td style='padding:8px;'>Wales 2026</td>"))
		out <- c(out, sprintf("<td style='text-align:right; padding:8px;'>%s</td>", fmt(net_w_26)))
		out <- c(out, sprintf("<td style='text-align:right; padding:8px; color:%s;'>%s (%s%.2f%%)</td>", 
